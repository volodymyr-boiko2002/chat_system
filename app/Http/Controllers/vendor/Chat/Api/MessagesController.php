...
class MessagesController extends Controller
{
    ...
    {
        $path = config('chat.attachments.folder') . '/' . $fileName;
        if (Chat::storage()->exists($path)) {
            return response()->json([
                'file_name' => $fileName,
                'download_path' => Chat::storage()->url($path)
            ], 200);
        } else {
            return response()->json([
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < Chat::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed)) {
                    ...
                    $file->storeAs(config('chat.attachments.folder'), $attachment, config('chat.storage_disk_name'));
                } else {
                    $error->status = 1;
    ...
    public function search(Request $request)
    {
        ...
        foreach ($records->items() as $index => $record) {
            $records[$index] += Chat::getUserWithAvatar($record);
        }
    ...
    {
        $images = Chat::getSharedPhotos($request['user_id']);
            ...
            if ($file->getSize() < Chat::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed_images)) {
                    ...
                    if (Auth::user()->avatar != config('chat.user_avatar.default')) {
                        $path = Chat::getUserAvatarUrl(Auth::user()->avatar);
                        if (Chat::storage()->exists($path)) {
                            Chat::storage()->delete($path);
                        }
                    }
                    // upload
